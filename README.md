...
스마트폰 앱은 Edge AI 기기에서 연산된 데이터를 수신하고 시각화하는 역할을 넘어, 하드웨어를 직접 제어하는 양방향 모니터이자 데이터 허브 역할을 수행합니다. 통신 안정성 확보와 사용자 친화적인 UX 제공을 위해 다음과 같은 아키텍처와 기능을 구현했습니다.
...
`![App Screenshot/GIF](이미지_경로_혹은_링크)`

**1. 로그인 및 다중 프로필 관리 (Auth & Multi-Profile)**
- **JWT 기반 인증:** Spring Boot 서버와 연동하여 안전한 로그인 및 회원가입 통신을 지원합니다.
- **가족 프로필 지원:** 하나의 앱으로 본인 및 가족 구성원의 건강 데이터를 독립적으로 측정하고 기록할 수 있는 다중 프로필 시스템을 구축했습니다.
- **로컬 보안 저장소:** `Secure Storage`를 활용해 프로필 정보와 인증 토큰을 기기 내부에 암호화하여 보관하며, 앱 재실행 시 로그인 상태를 유지합니다.

**2. 양방향 블루투스(BLE) 통신 및 기기 제어 (Two-way BLE)**
- **Write/Notify 병렬 통신:** 단순히 데이터를 수신(Notify)하는 것에 그치지 않고, 쓰기 특성(Write Characteristic)을 통해 앱에서 기기의 측정 프로세스를 원격으로 제어(Trigger)하는 양방향 시스템을 구현했습니다.
- **연결 흐름 최적화:** 기기 스캔 중 발생할 수 있는 무한 로딩을 방지하기 위해 타임아웃을 설정하고, 미연결 상태에서 측정 시도 시 연결 안내 팝업을 통해 설정 화면으로 유도하는 UX를 적용했습니다.

**3. 원격 측정 제어 및 실시간 시각화 (Remote Control & Real-time UI)**
- **App-Driven UX:** 사용자가 기기의 버튼을 조작할 필요 없이 앱 내 측정 시작 버튼을 누르면 블루투스 명령이 전송되어 기기의 카운트다운과 측정 로직이 실행됩니다.
- **동적 상태 관리:** 블루투스 비활성화, 기기 탐색, 측정 대기, 실시간 수신, 결과 도출로 이어지는 5단계 상태 변화를 `AnimatedSwitcher`를 활용해 직관적으로 전환합니다.
- **알고리즘 투명성:** 측정 완료 시 산소포화도(SpO2)와 심박수(BPM)뿐만 아니라, AI가 판별한 피부톤 보정 등급(Tier)을 화면에 명시하여 데이터 보정 과정을 사용자에게 투명하게 공개합니다.

**4. 대시보드 및 데이터 파이프라인 (Dashboard & History)**
- **측정 기록 요약:** 홈 화면에서 선택된 프로필의 최신 측정 기록을 한눈에 확인할 수 있는 요약 카드를 제공합니다.
- **데이터 초기화 (Hard Reset):** 서버 저장 완료 시 결과 변수를 즉시 강제 초기화하고 기기 상태를 재점검하여 연속적인 재측정이 가능하도록 사이클을 최적화했습니다.
- **영구 데이터 적재:** 측정이 완료된 데이터는 HTTP 통신을 통해 Spring Boot 백엔드 서버 DB에 영구 저장됩니다.

**5. 설정 및 개인정보 보호 (Settings & Data Privacy)**
- **직관적인 기기 관리:** 프로필 전환과 블루투스 기기 재연결에 집중한 심플한 UI를 구성했습니다.
- **계정 및 데이터 파기:** 회원 탈퇴 시 백엔드 DB에 저장된 해당 사용자의 모든 측정 데이터를 일괄 삭제(Cascade Remove)하여 개인정보 보호 원칙을 철저히 준수합니다.

---

#### 🎯 Troubleshooting

**🔥 Issue 1: 측정 탭 재진입 시 이전 측정 데이터가 즉시 표출되는 현상**
- **원인:** 블루투스 라이브러리의 `lastValueStream` 특성상, 스트림 재구독 시 기기에 남아있던 이전 캐시(Cache) 데이터를 즉시 방출하여 발생하는 생명주기 문제였습니다.
- **해결:** 과거 값을 무시하고 새로 수신된 실시간 데이터만 구독하는 `onValueReceived` 리스너로 마이그레이션하여 데이터 무결성을 확보했습니다.
