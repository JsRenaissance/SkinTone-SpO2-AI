...
## 📱 9. 모바일 앱 주요 화면 및 기능 (Mobile App UI & Features)

스마트폰 앱은 아두이노(Edge AI)에서 연산된 데이터를 수신하고 시각화하는 **'모니터이자 데이터 허브'** 역할을 수행합니다. 통신 안정성을 확보하고, 사용자 친화적인 UX를 제공하기 위해 다음과 같은 핵심 기능들을 구현했습니다.


### 👥 1. 로그인 및 다중 프로필 관리 (Auth & Multi-Profile)
* **JWT 기반 인증:** 안전한 로그인 및 회원가입 통신을 지원합니다.
* **가족 프로필 지원:** 하나의 기기(앱)로 여러 명(예: 본인, 어머니, 아버지)의 건강을 독립적으로 측정하고 기록할 수 있는 다중 프로필 시스템을 구축했습니다.
* **로컬 금고 암호화:** `Secure Storage`를 활용해 프로필 정보와 인증 토큰을 스마트폰 내부에 안전하게 보관하여 앱 재실행 시 상태를 유지합니다.

### 📡 2. 블루투스(BLE) 연동 및 UX 최적화 (Device Connection)
* **단방향 통신 최적화:** 앱이 아두이노를 직접 제어하지 않고, 아두이노의 측정이 끝날 때까지 기다리는 '수신 대기' 방식을 채택하여 하드웨어 통신 충돌과 데이터 파편화를 방지했습니다.
* **안전한 연결 흐름 (Escape Hatch):** 기기 스캔 중 무한 로딩(프리징)에 빠지지 않도록 타임아웃(Timeout)을 설정하고, 명시적인 '돌아가기' 탈출구를 마련하여 UX를 개선했습니다.
* **스마트 라우팅:** 기기가 연결되지 않은 상태에서 측정을 시도하면, 즉시 '기기 연결 안내 팝업'을 띄워 자연스럽게 블루투스 설정 화면으로 유도합니다.

### 📈 3. 실시간 측정 및 시각화 (Real-time Measurement)
* **동적 UI 전환:** `측정 대기 ➔ 실시간 측정(수신 대기) ➔ 결과 도출`의 3단계 과정을 부드러운 상태 전환 애니메이션으로 표출합니다.
* **피부톤 캘리브레이션 투명성:** 측정 완료 시 산소포화도(SpO2), 심박수(BPM) 결과와 함께 **아두이노 AI가 판별한 피부톤 Tier(보정 등급)**를 화면에 명시하여, 알고리즘의 동작을 사용자가 직접 확인할 수 있게 합니다.

### 📊 4. 대시보드 및 건강 기록 (Dashboard & History)
* **최근 상태 요약:** 홈 화면에서 현재 선택된 프로필 사용자의 가장 최근 측정 기록을 한눈에 확인할 수 있는 요약 카드를 제공합니다.
* **데이터 영구 적재:** 측정이 성공적으로 완료되면 스마트폰의 인터넷망(HTTP/Retrofit)을 이용해 Spring Boot 백엔드 서버로 데이터를 전송하고 영구 저장합니다.
