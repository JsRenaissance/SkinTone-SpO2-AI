...
* [**📊 상세 데이터 분석 리포트 (HTML 확인)**](https://htmlpreview.github.io/?https://github.com/JsRenaissance/SkinTone-SpO2-AI/blob/main/11k.html)

<br>

아이고, 올려주신 캡처 화면을 보니 문단이 다닥다닥 다 붙어버리고 수식 기호($$, \frac 등)가 그대로 노출되어서 엉망으로 렌더링되었네요!

문제가 발생한 이유는 크게 두 가지입니다.

마크다운(Markdown) 줄바꿈 규칙: 지금 사용하시는 에디터나 뷰어(깃허브, 노션 등)가 엔터(Enter) 한 번을 줄바꿈으로 인식하지 못하고 있습니다. 마크다운에서는 엔터를 두 번 쳐서 완전히 한 줄을 띄우거나, 문장 끝에 스페이스바를 두 번 넣어야 줄바꿈이 적용됩니다.

수식(LaTeX) 뷰어 미지원: 제가 적어드린 수학 공식 포맷(LaTeX)을 현재 글을 작성하고 계신 플랫폼에서 지원하지 않아서, 수식을 그림으로 변환하지 못하고 코드 그대로 뱉어낸 것입니다.

졸업작품 포트폴리오나 깃허브 README 등 어디에 올려도 절대 깨지지 않도록, 복잡한 수식 코드를 모두 일반 텍스트와 기호(α 등)로 바꾸고 줄바꿈을 확실하게 적용한 유니버셜 버전으로 다시 정리해 드릴게요.

아래 텍스트를 그대로 복사해서 다시 붙여넣어 보세요!

🧮 6. 알고리즘 이식 및 보정 로직 (Implementation & Calibration)
기존 센서의 경험식에 AI가 도출한 피부톤 보정 계수를 결합하여, 하드웨어 단에서 즉각적으로 오차를 교정하는 3단계 수학적 파이프라인을 구축했습니다.

🩸 Phase 1. 기본 광학 산소포화도 도출
가장 먼저 MAX30102 센서를 통해 심장 박동에 따른 혈류량 변화를 파악하고, 적색광(Red)과 적외선(IR)의 빛 흡수율 비율(R)을 계산합니다.

R = (AC_Red / DC_Red) / (AC_IR / DC_IR)

도출된 R 값을 센서 제조사(Maxim Integrated)의 임상 실험 기반 캘리브레이션 1차 방정식에 대입하여, 피부톤 오차가 포함된 기본 산소포화도(SpO2_Raw)를 1차 산출합니다.

SpO2_Raw = 104 - (17 * R)

104 (A): 빛 흡수율 비율이 0일 때의 이론적 최대 산소포화도 상수

17 (B): R 값 증가에 따른 산소포화도 감소 기울기 상수

⚖️ Phase 2. Edge AI 기반 피부톤 가중치 (W_skin) 산출
이 프로젝트의 핵심인 피부톤 왜곡 현상을 소프트웨어적으로 해결하는 단계입니다. 앞서 학습시킨 머신러닝 모델(Decision Tree)을 통해 현재 측정자의 피부톤 티어(Skin Tier)를 실시간으로 분류하고, 이에 따른 동적 보정 가중치(W_skin)를 산출합니다.

W_skin = α * (C_skin - C_ref)

C_skin: 컬러 센서(TCS34725)로 실시간 측정한 현재 사용자의 피부톤 변환 데이터 (명도 및 RGB 비율)

C_ref: 빛 흡수 간섭이 0에 수렴하는, 영점 조절(Calibration)된 가장 밝은 표준 피부톤 상수

α (보정 계수): 실제 상용 의료용 펄스옥시미터(Ground Truth)와 MAX30102 센서 간의 오차 데이터를 수집한 뒤, 선형 회귀 분석(Linear Regression)을 통해 실험적으로 도출해 낸 기울기 가중치

🎯 Phase 3. 최종 산소포화도 확정
분류된 피부톤 티어(Fair, Medium, Dark)에 맞춰 산출된 가중치를 기본 측정값에 결합하여, 멜라닌 색소의 방해를 걷어낸 최종 캘리브레이션 수치를 확정합니다.

SpO2_Final = SpO2_Raw + W_skin

Tier 1 (Fair): 멜라닌 영향이 적어 W_skin ≈ 0 (기본값 유지)

Tier 2 (Medium): 중간 농도 멜라닌 보정을 위한 1단계 α 가중치 연산

Tier 3 (Dark): 고농도 멜라닌 광 신호 왜곡을 상쇄하기 위한 최대 α 가중치 연산
---
